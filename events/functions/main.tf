...
data "archive_file" "bundle" {
  ...
  output_path = "${path.module}/.build/events-bridge.zip"
}

resource "google_storage_bucket" "gcf_source" {
  name                        = "${var.project_id}-events-fn"
  location                    = var.region
  uniform_bucket_level_access = true
}

resource "google_storage_bucket_object" "bundle" {
  name   = "events-bridge-${data.archive_file.bundle.output_md5}.zip"
...
locals {
  events_env = merge(
    ...
    var.events_internal_gateway_hostname != "" ? {
      ACS_EVENTS_GATEWAY_HOSTNAME = var.events_internal_gateway_hostname
    } : {}
  )
}
...
resource "google_cloudfunctions2_function" "bridge" {
  ...
  service_config {
    ...
    environment_variables            = local.events_env
  }
}
